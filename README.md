...
### 4.1 Componentes de Presentación Compartidos (`common/presentation/components/`)
...
### 4.2 Utilidades Compartidas (`common/utils/`)
...
## 6. Regla Crítica: Prevención de Lag y Duplicación en Inputs ⌨️
...
### 9.1 `google-services.json` (Obligatorio para builds nativos)
...
### 9.2 Variables de Entorno en EAS Builds (Crítico)
...
A continuación se listan todos los comandos necesarios para instalar, configurar, ejecutar y compilar el frontend.
...
### 10.2 Configuración
* Crear el archivo `.env` en la raíz del proyecto basándose en `env.example` y configurar las variables.
...
* **Correr directamente en emulador Android:**
  ```bash
  npm run android
  ```
* **Correr directamente en emulador iOS:**
  ```bash
  npm run ios
  ```

### 10.4 Comandos de Compilación (EAS Build)
* **Generar APK para Android (Perfil Preview):**
  ```bash
  eas build --profile preview --platform android
  ```
* **Firma y obtención de SHA-1 (Para Google Sign-In nativo):**
  ```bash
  eas credentials --platform android
  ```
  *(Selecciona el perfil `preview` para registrar el SHA-1 obtenido en la Google Cloud Console junto al package name `com.tinyfood.app`).*
